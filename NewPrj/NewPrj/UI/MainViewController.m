...
- (void)viewDidLoad
{
    ...
    UIView *viewLeft= [[UIView alloc]initWithFrame:CGRectMake(0, 0, 50, 44)];
    viewLeft.backgroundColor = [UIColor clearColor];
    UIButton *leftButton =  [UIButton buttonWithType:UIButtonTypeCustom];
    
    [leftButton addTarget:self action:@selector(uploadAction) forControlEvents:UIControlEventTouchUpInside];
    [leftButton setFrame:CGRectMake(0,0,50,44)];
    
    UIImageView *leftImgView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"upload.png"]];
    leftImgView.frame = CGRectMake(7,10,32,32);
    [viewLeft addSubview:leftImgView];
    [viewLeft addSubview:leftButton];
    UIBarButtonItem *leftItem = [[UIBarButtonItem alloc] initWithCustomView:viewLeft];
    ...
    imgView.frame = CGRectMake(7,10,32,32);
...
- (void)uploadAction
{
    ...
    if (([loctionInfo objectForKey:@"contentLbl"] == nil)&&([photoInfo objectForKey:@"contentImageView"] == nil)) {
        UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"Notice" message:@"Make sure you have got the photo and locaiton" delegate:nil cancelButtonTitle:@"Yes" otherButtonTitles:nil, nil];
        [alert show];
        return;
    }else{
        if ([photoInfo objectForKey:@"contentImageView"] == nil) {
            UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"Notice" message:@"Make sure you have got the photo" delegate:nil cancelButtonTitle:@"Yes" otherButtonTitles:nil, nil];
            
            [alert show];
            return;
        }
        if ([loctionInfo objectForKey:@"contentLbl"] == nil) {
            UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"Notice" message:@"Make sure you have got the locaiton" delegate:nil cancelButtonTitle:@"Yes" otherButtonTitles:nil, nil];
...
{
    if (TARGET_IPHONE_SIMULATOR) {
        //because the simulator return a wrong gps string ,So I give a right gps coordinate sting to make sure the upload can be finsihed
        [loctionInfo setObject:@"121.550223,38.859400" forKey:@"contentLbl"];
    }else{
