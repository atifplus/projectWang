...
//内部方法---end
- (void)updateRecordWithInfo:(NSDictionary *)surveyInfo;
{
    NSManagedObject* object;
    if (currentPic == nil) {
         object = [NSEntityDescription insertNewObjectForEntityForName:@"Picture" inManagedObjectContext:self.managedObjectContext];
         currentPic = (Picture *)object;
    }else{
        object = currentPic;
    }
    [self setObj:object WithDict:surveyInfo];
...
//    return percent;
//}
- (NSArray *)searchRecordsWithOneFieldName:(NSString*)name
{
    
    NSFetchRequest *request = [self setEntity:@"Picture" sortStr:@"name" field:@"name" filter:name isASC:NO];
    NSError *error;
    NSMutableArray *fetchResults = [[self.managedObjectContext executeFetchRequest:request error:&error] mutableCopy];
    int count = [fetchResults count];
    NSMutableArray *recordList = [[NSMutableArray alloc] init];
    NSArray *keys = [NSArray arrayWithObjects:@"name",@"location",@"image", nil];
    if (count>0)   {
        for (NSManagedObject *obj in fetchResults) {
            [recordList addObject:[self getDictfromObj:obj withKeys:keys]];
        }
    }else{
        NSLog(@"没没找到调研信息");
    }
    return [recordList mutableCopy];
}
